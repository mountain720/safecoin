...
#ifndef SAFECOIN_NSPVWALLET_H
#define SAFECOIN_NSPVWALLET_H

// nSPV wallet uses superlite functions (and some safecoind built in functions) to implement nSPV_spend
...
int32_t NSPV_gettransaction(int32_t skipvalidation,int32_t vout,uint256 txid,int32_t height,CTransaction &tx,uint256 &hashblock,int32_t &txheight,int32_t &currentheight,int64_t extradata,uint32_t tiptime,int64_t &rewardsum)
{
    ...
    {
        rewards = safecoin_interestnew(height,tx.vout[vout].nValue,tx.nLockTime,tiptime);
...
bool NSPV_SignTx(CMutableTransaction &mtx,int32_t vini,int64_t utxovalue,const CScript scriptPubKey,uint32_t nTime)
{
    ...
    if ( nTime != 0 && nTime < SAFECOIN_SAPLING_ACTIVATION )
    {
