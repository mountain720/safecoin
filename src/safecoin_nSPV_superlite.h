...
#ifndef SAFECOIN_NSPVSUPERLITE_H
#define SAFECOIN_NSPVSUPERLITE_H
...
// safecoin_nSPVresp is called from async message processing

void safecoin_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// safecoin_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void safecoin_nSPV(CNode *pto) // polling loop from SendMessages
{
...
UniValue NSPV_getinfo_json(struct NSPV_inforesp *ptr)
{
    ...
    result.push_back(Pair("result","success"));
    result.push_back(Pair("nSPV",SAFECOIN_NSPV==-1?"disabled":(SAFECOIN_NSPV_SUPERLITE?"superlite":"fullnode")));
...
#endif // SAFECOIN_NSPVSUPERLITE_H
