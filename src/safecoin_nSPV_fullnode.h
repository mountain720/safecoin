...
#ifndef SAFECOIN_NSPVFULLNODE_H
#define SAFECOIN_NSPVFULLNODE_H
...
int32_t NSPV_notarization_find(struct NSPV_ntzargs *args,int32_t height,int32_t dir)
{
    ...
    symbol = (ASSETCHAINS_SYMBOL[0] == 0) ? (char *)"SAFE" : ASSETCHAINS_SYMBOL;
...
int32_t NSPV_ntzextract(struct NSPV_ntz *ptr,uint256 ntztxid,int32_t txidht,uint256 desttxid,int32_t ntzheight)
{
    ...
    ptr->txid = ntztxid;
    if ( (pindex= safecoin_chainactive(ptr->txidheight)) != 0 )
...
int32_t NSPV_getinfo(struct NSPV_inforesp *ptr,int32_t reqheight)
{
    ...
    if ( (pindex= chainActive.LastTip()) != 0 )
    {
        ...
        if ( (pindex2= safecoin_chainactive(ptr->notarization.txidheight)) != 0 )
            ...
            for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
            {
                // if gettxout is != null to handle mempool
                {
                    if ( n >= skipcount && myIsutxo_spentinmempool(ignoretxid,ignorevin,it->first.txhash,(int32_t)it->first.index) == 0  )
                    {
                        ...
                        {
                            ptr->utxos[n].extradata = safecoin_accrued_interest(&txheight,&locktime,ptr->utxos[ind].txid,ptr->utxos[ind].vout,ptr->utxos[ind].height,ptr->utxos[ind].satoshis,tipheight);
...
int32_t NSPV_gettxproof(struct NSPV_txproof *ptr,int32_t vout,uint256 txid,int32_t height)
{
    ...
    if ( (ptr->tx= NSPV_getrawtx(_tx,hashBlock,&ptr->txlen,txid)) != 0 )
    {
        ...
        if ( height == 0 )
            ptr->height = safecoin_blockheight(hashBlock);
        else
        {
            ptr->height = height;
            if ( (pindex= safecoin_chainactive(height)) != 0 && safecoin_blockload(block,pindex) == 0 )
            {
...
void safecoin_nSPVreq(CNode *pfrom,std::vector<uint8_t> request) // received a request
{
