...
cJSON *get_safecoincli(char *refcoin,char **retstrp,char *acname,char *method,char *arg0,char *arg1,char *arg2,char *arg3)
{
    ...
    //if ( (acname == 0 || acname[0] == 0) && strcmp(refcoin,"SAFE") != 0 )
    //    acname = refcoin;
    if ( acname[0] != 0 )
    {
...
int32_t itemvalid(char *refcoin,int64_t *refundedp,int64_t *waitingp,struct claimitem *item)
{
    ...
    *refundedp = *waitingp = 0;
    ...
    sprintf(url,"https://safe.explorer.dexstats.info/insight-api-safecoin/addr/%s",item->destaddr);
...
void reconcile_claims(char *refcoin,char *fname)
{
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        while ( fgets(buf,sizeof(buf),fp) > 0 )
        {
            ...
            printf("%s\n",fields[1]);
            total += update_claimstats(fields[1],fields[3],fields[5 + (strcmp("SAFE",refcoin)==0)],atof(fields[4])*SATOSHIDEN + 0.0000000049);
            numlines++;
        }
        fclose(fp);
    }
            ...
            if ( (n= cJSON_GetArraySize(retjson)) > 0 )
            {
                for (i=0; i<n; i++)
                {
                    ...
                    if ( (addr= jstr(item,"address")) != 0 && strcmp(addr,"RWXL82m4xnBTg1kk6PuS2xekonu7oEeiJG") == 0 )
                    {
                        ...
                        if ( strcmp(coinstr,"SAFE") == 0 && verify_vin(coinstr,jbits256(item,"txid"),0,"R9JCEd6xnCxNUSpLrHEWvzPSh7CNXm7z75") < 0 )
                        {
                            printf("WARNING: imposter dust detected! %s\n",bits256_str(str,jbits256(item,"txid")));
                            continue;
                        }
                        else if ( strcmp(coinstr,"SAFE") != 0 && verify_vin(coinstr,jbits256(item,"txid"),0,"R9MUnxXijovvSeT9sFuUX23TiFtVvZEGjT") < 0 )
                        {
                            printf("WARNING: imposter dust detected! %s\n",bits256_str(str,jbits256(item,"txid")));
                            continue;
                        }
