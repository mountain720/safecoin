...
./safecoin-cli -ac_name=ROGUE cclib register 17 \"[%224fd6f5cad0fac455e5989ca6eef111b00292845447075a802e9335879146ad5a%22,%22playerdata_txid%22]\"

 If you want to cash in your ingame gold and preserve your character for another battle, do the bailout:
 
./safecoin-cli -ac_name=ROGUE cclib bailout 17 \"[%224fd6f5cad0fac455e5989ca6eef111b00292845447075a802e9335879146ad5a%22]\"

 If you won your game before anybody else did or if you are the last one left who didnt bailout, you can claim the prize:
 
 ./safecoin-cli -ac_name=ROGUE cclib highlander 17 \"[%224fd6f5cad0fac455e5989ca6eef111b00292845447075a802e9335879146ad5a%22]\"
...
// ./safecoin-cli -ac_name=ROGUE cclib register 17 \"[%22a898f4ceef7647ba113b9f3c24ef045f5d134935a3b09bdd1a997b9d474f4c1b%22,%22f11d0cb4e2e4c21f029a1146f8e5926f11456885b7ab7d665096f5efedec8ea0%22]\"
...
int32_t rogue_isvalidgame(struct CCcontract_info *cp,int32_t &gameheight,CTransaction &tx,int64_t &buyin,int32_t &maxplayers,uint256 txid,int32_t unspentv0)
{
    ...
    {
        if ( txid != zeroid )
            gameheight = safecoin_blockheight(hashBlock);
        else
        {
            txid = tx.GetHash();
    ...
    if ( matches == 1 )
    {
        ...
        if ( rogue_registeropretdecode(txid,tokenid,playertxid,matchtx.vout[numvouts-1].scriptPubKey) == 'R' )//&& txid == gametxid )
        {
            ...
            if ( active == zeroid || rogue_playerdata(cp,origplayergame,tid,pk,playerdata,symbol,pname,active) == 0 )
            {
                ...
                {
                    if ( hashBlock == zeroid )
                        batonht = safecoin_nextheight();
                    else if ( (pindex= safecoin_blockindex(hashBlock)) == 0 )
                        return(-4);
...
int32_t rogue_playersalive(int32_t &openslots,int32_t &numplayers,uint256 gametxid,int32_t maxplayers,int32_t gameht,CTransaction gametx)
{
    ...
    if ( safecoin_nextheight() <= gameht+ROGUE_MAXKEYSTROKESGAP )
        registration_open = 1;
    for (i=0; i<maxplayers; i++)
    {
        ...
        if ( CCgettxout(gametxid,1+i,1,0) < 0 )
        {
            ...
            if ( CCgettxout(gametxid,1+maxplayers+i,1,0) == txfee )
            {
                ...
                if ( txid != zeroid )
                {
                    ...
                    {
                        if ( (pindex= safecoin_blockindex(hashBlock)) != 0 )
                        {
...
uint64_t rogue_gamefields(UniValue &obj,int64_t maxplayers,int64_t buyin,uint256 gametxid,char *myrogueaddr)
{
    ...
    if ( myGetTransaction(gametxid,tx,hashBlock) != 0 && (pindex= safecoin_blockindex(hashBlock)) != 0 )
    {
        ht = pindex->GetHeight();
        delay = ROGUE_REGISTRATION * (maxplayers > 1);
        obj.push_back(Pair("height",ht));
        obj.push_back(Pair("start",ht+delay));
        if ( safecoin_nextheight() > ht+delay )
        {
            if ( (pindex= safecoin_chainactive(ht+delay)) != 0 )
            {
                ...
                else sprintf(cmd,"./safecoin-cli -ac_name=%s cclib register %d \"[%%22%s%%22]\"",ASSETCHAINS_SYMBOL,EVAL_ROGUE,gametxid.ToString().c_str());
    ...
    if ( params != 0 && (n= cJSON_GetArraySize(params)) > 0 )
    {
        if ( n > 0 )
        {
            ...
            if ( (err= rogue_isvalidgame(cp,gameheight,tx,buyin,maxplayers,gametxid,1)) == 0 )
            {
                ...
                if ( safecoin_nextheight() > gameheight + ROGUE_MAXKEYSTROKESGAP )
                ...
                if ( rogue_findbaton(cp,playertxid,&keystrokes,numkeys,regslot,playerdata,batontxid,batonvout,batonvalue,batonht,gametxid,gametx,maxplayers,myrogueaddr,numplayers,symbol,pname) == 0 )
                {
                    ...
                    if ( num > 0 )
                    {
                        ...
                        else
                        {
                            ...
                            if ( safecoin_nextheight() > 77777 && cashout > ROGUE_MAXCASHOUT )
...
UniValue rogue_pending(uint64_t txfee,struct CCcontract_info *cp,cJSON *params)
{
    ...
    nextheight = safecoin_nextheight();
